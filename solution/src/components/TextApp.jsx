...
function TextApp() {
    ...
    async function handleSend(e) {
        ...
        if(input) {
            ...
            const resp = await fetch("https://cs571api.cs.wisc.edu/rest/f25/hw11/completions", {
                method: "POST",
                headers: {
    ...
    async function handleSendAsStream(e) {
        ...
        if(input) {
            addMessage(Constants.Roles.User, input);
            inputRef.current.value = "";
            const resp = await fetch("https://cs571api.cs.wisc.edu/rest/f25/hw11/completions-stream", {
                method: "POST",
                headers: {
                    "X-CS571-ID": CS571.getBadgerId(),
                    "Content-Type": "application/json",
